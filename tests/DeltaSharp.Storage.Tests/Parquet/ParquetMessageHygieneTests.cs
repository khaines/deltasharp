...
public sealed class ParquetMessageHygieneTests
{
    ...
    public void CreateField_NestedColumn_EchoesBoundedKindNotRecursiveSimpleString()
    {
        ...
        // #873 LIFTED the #585 nested-within-nested reject (struct-of-struct now maps), so the poison rides a
        // shape that STILL fails closed — a struct whose LAST field is a map whose KEY is a container (#873 D5,
        // nested map key). The 5,000 sibling fields make an unbounded render (which would recurse into every
        // one of them) impossible to miss.
        var fields = new StructField[5_000];
        for (int i = 0; i < fields.Length - 1; i++)
        {
            fields[i] = new StructField(
