...
/// strings requiring JSON escaping in all three positions that carry them (field name, metadata
/// key, metadata string value) — and three completeness guards, each deriving its
...
public sealed class DeltaSchemaJsonSingleSourceTests
{
    ...
    public void EveryStorageTypeTouchingUtf8JsonWriter_IsOnTheJsonWritingAllowlist()
    {
        ...
        // included), every atomic type the writer accepts (decimal boundaries included) and strings
        // requiring JSON escaping in every position that carries one (field name, metadata key,
        // metadata string value), with three completeness guards that fire if the corpus
