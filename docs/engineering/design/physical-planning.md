...
feeds input through an in-memory relation fixture (§7).

---

## 2. The Core ↔ Executor seam (STORY-04.6.1 / #173)

The sibling lane #173 owns three Core types; this lane implements the executor behind them. #173 is
now **merged into `main`**, so this branch consumes the **real** Core seam directly (its earlier
minimal stand-ins were dropped at rebase — see §10); this PR adds **no** Core-side changes.

| Core type | Role | Owner |
| --- | --- | --- |
| `public sealed class DeltaSharp.Row` | A materialized result row: `Schema`, ordinal/by-name access, `GetAs<T>`, `IsNullAt`, null-aware. | #173 (merged) |
...
**Rebase note:** this duplicates analyzer internals. The durable fix is an exposed output seam on the
analyzer/optimizer; when #172 (optimizer) and #173 land, replace `LogicalOutput` with the real seam
(see §9).
...
| Integer, Date | `int` | `GetValue<int>` |
| Long, Timestamp | `long` | `GetValue<long>` |
| Float / Double | `float` / `double` | `GetValue<…>` |
| Decimal | `decimal` (best-effort) | compact → `long`, else `Int128`, unscaled ÷ 10^scale |
...
materialization). Date/Timestamp surface as `int`/`long` epoch values in M1 (richer temporal
surfacing is deferred); large-magnitude decimals are best-effort.
...
- an attribute that does not resolve against its input (ExprId drift) — the translator;
...
## 10. Rebase reconciliation points (#172 optimizer, #173 Row/IQueryExecutor)

This PR was implemented against the #173 contract before #172/#173 merged. Both are now merged into
...
1. **Row / IQueryExecutor / SparkSession hook — done.** The Core stand-ins this branch once carried
   ...
   grant, and the `PublicAPI.Unshipped.txt` `Row` entries) were **dropped**; the branch now binds to
   #173's merged seam: `DeltaSharp.Row`, `DeltaSharp.Execution.IQueryExecutor`,
   `SparkSession.RegisterQueryExecutorFactory`, and the fail-closed `UnsupportedQueryExecutor` default.
   The `PublicAPI.*.txt` diff owned by *this* PR is empty (those entries belong to #173).
2. **`LogicalOutput` ExprId reconstruction (§5).** Replace with the real analyzer/optimizer output
   seam once available. Verify the optimizer (#172) preserves attribute ExprIds; if it mints/reorders
   outputs, the self-checking resolution will flag drift loudly (by design) — that is the signal to
   switch to the exposed seam.
3. **AnsiMode / backend selection.** Confirm the merged `SparkSession` execution-backend config
   (`spark.deltasharp.execution.backend`) still maps as in `LocalQueryExecutor.OptionsFor`.
