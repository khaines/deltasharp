...
- **AC2 — documented omit/bound rules** (single source of truth: `StatisticsPolicy`, default `MaxIndexedColumns=32`, `StringTruncationLength=32`):
  ...
  3. **String truncation** — string min/max are prefix-truncated to `StringTruncationLength` Unicode characters (never splitting a surrogate pair); **any** truncation sets the file's `tightBounds=false`, which disables numeric range-pruning for that file (conservative — see deviations).
  4. **All-null / empty column** — `nullCount` is recorded but min/max are omitted (an all-null or empty column has no bound).
  5. **Non-finite floats** — `NaN` is excluded from bounds (it never satisfies a comparison), `-0.0` is canonicalized to `+0.0`, and a `±Infinity` bound is omitted (no JSON-number encoding).
  Stat **values are user data** and are never emitted into logs/telemetry (§5.1, §7.4) — only coverage/shape.
- **AC3 — skipping is an optimization, never correctness.** `Snapshot.PruneFiles(request)` → `FilePruningResult { Candidates, TotalFiles, Skipped }`, where each `SkippedFile` carries a `FileSkipReason` (`PartitionMismatch` or `StatisticsDisjoint`) as an **audit trail**. `Candidates` is always a sound over-approximation, so `Candidates ∪ Skipped` exactly partitions the input and the residual predicate is always re-evaluated at scan. The pruner is **fail-open**: absent stats, `tightBounds=false`, non-numeric (`String`/`Boolean`) bounds, or any un-comparable kind ⇒ **keep** (scan) the file; only tight, numeric, provably-disjoint ranges (or an all-null column) produce a skip.
- **AC4 — optimizer statistics API.** `Snapshot.GetStatistics(policy?)` → `TableStatisticsReport` (read-only, side-effect-free) carrying a **freshness/version token** (`Version`/`FreshnessToken` = the snapshot version), table aggregates (`FileCount`, `TotalSizeInBytes`, and `RecordCount` populated **only** when `HasCompleteRecordCounts` — every file recorded `numRecords`), and per-file `FileStatisticsReport`s. Each column's availability is an explicit **absence reason** (`StatisticColumnState`): `Available`, `AbsentNoStatistics`, `AbsentNotIndexed`, `AbsentAllNull`, `AbsentNonTightBounds`, `OmittedNestedType`, `OmittedUnsupportedType` — so an optimizer distinguishes a missing hint from a data condition and never treats a hint as a correctness input.

**Deviations & follow-ups** (tracked, not blocking): (a) any string truncation sets the whole file's `tightBounds=false` rather than applying Delta's per-value **min-down/max-up** tie-break — safe (it only disables an optimization) but coarser than Delta; a future change can adopt the tie-break to keep numeric pruning tight alongside truncated strings. (b) `date`/`timestamp` bounds are stored as numeric lane values (`Long`, prunable) rather than Delta's ISO-8601 strings — internally consistent and prunable, but a cross-engine ISO encoding is deferred. (c) `decimal` bounds are stored as `String` and are therefore not used for range pruning (sound; forfeits that optimization). (d) End-to-end wiring of `WriteWithStatisticsAsync` into the production write path depends on the deferred write-door (#487); the extractor, policy, skip-reporting, and stats API are complete and unit/integration-tested independently.
