...
- **`typeWidening` table feature → in-place widening (#495).** When the table registers Delta's `typeWidening`
  ...
  **create-time** (feature declared in `protocol` + property set at table
  ...
  `TypeWideningUnsupported`). `date→timestamp_ntz` is a **read-only** widening (**#533**): DeltaSharp adds a
  `TimestampNtzType` (Spark `timestamp_ntz`, INT64 micros, timezone-less) and **read-promotes** an older
  `date` file to `timestamp_ntz` (INT32 epoch-day → INT64 midnight-of-date micros), but does **not** apply it
  on append — it cannot yet **write** a native `timestamp_ntz` Parquet column because Parquet.Net 6.0.3 cannot
  persist the `isAdjustedToUTC=false` annotation (it emits a legacy UTC `TIMESTAMP_MICROS` ConvertedType), so a
  written column would be a protocol-non-conformant LTZ file; native `timestamp_ntz` writes are **fail-closed**
  and an append widening is rejected as `TypeWideningUnsupported` (like the cross-family cases). `date→timestamp`
  with a **timezone** (LTZ) is NOT a sanctioned widening at all — Delta only widens `date→timestamp_ntz` — so it
  is a plain `IncompatibleType`. The cross-family (`int→double`, `int/long→decimal`) and nested
