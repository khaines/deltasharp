...
The active file set is **derived from committed log versions + checkpoints, never from a directory listing** (checklist *Delta log protocol* bullet 3; anti-pattern #1). Replay depth, checkpoint age, and action counts are surfaced as metrics with bounded memory (checklist *Snapshot* bullet 6). When a **selected** checkpoint is discarded while seeding step 3 — an encrypted checkpoint the reader cannot read (`unsupported_feature`, #681/#698) or a malformed one (`DeltaProtocolException`, including the forged multi-`metaData` reject below) — the discard emits the structured `deltasharp.delta.checkpoint.fallbacks{reason}` counter + a `Warning` log (`DeltaCheckpointLog.CheckpointFallback`, EventId 4400) carrying the discarded checkpoint version (#772), so an otherwise-silent fallback to an older checkpoint / full JSON replay is recoverable from telemetry. Selection-time skips at step 2 (an incomplete multi-part group, a V2/UUID checkpoint, or a failed `_last_checkpoint` hint) are a separate, often-benign path and are not counted.
