...
| Partition columns | A partition column's **data type cannot be evolved in place** here. A *non-widening* type change (narrowing/unrelated) is rejected explicitly as `PartitionColumnEvolutionUnsupported` — clearer than the generic type rejection. **Any** Delta-sanctioned **widening** — intra-family (byte→…→long, float→double, grow-only decimal), cross-family (integral→double/decimal, **#535**), or date→timestamp_ntz (**#533**) — is a distinct, **honest** case: Delta **does** sanction partition-column widening **without a table rewrite** (partition values are stored as strings in the log / directory names — verified vs Delta's golden `TypeWideningAlterTableSuite`), but this build **defers** it (**#537**) — rejected fail-closed as `TypeWideningUnsupported` with a message that says so, **not** the factually-wrong "requires a full table rewrite" reason. This classification is **independent of `delta.enableTypeWidening`**: a disabled feature is a feature-enablement gap, not a layout rewrite, so the widening deferral (never the rewrite reason) applies whether or not widening is enabled. Adding *data* columns is allowed. |
