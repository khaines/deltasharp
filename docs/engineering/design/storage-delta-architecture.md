...
- **Dry-run** reports the plan (which files *would* be compacted) without writing or committing. **Bounded memory (v1):** a group's rows are materialized before its output is written (bounded by the target file size); streaming rewrite and clustering-aware / Z-order compaction are tracked follow-ups.
