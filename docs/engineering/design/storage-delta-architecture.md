...
- **What lands where.** The **bulk of the epic is engine-internal** and lands in `DeltaSharp.Storage`
  ...
  `DeltaTable` handle.
- **Unsafe/AOT posture.** `DeltaSharp.Storage` sets `AllowUnsafeBlocks=true` (scoped project-local, same
  rationale as `DeltaSharp.Engine.csproj` — `Span<byte>` over `NativeMemory` pointers on the Parquet
  decode path) and stays trim/AOT-annotation-clean (ADR-0014), because it flows into the Executor
  NativeAOT image; Parquet decode must use **no dynamic code** (no `Reflection.Emit`/`Expression.Compile`
  — banned by `BannedSymbols.txt`).
...
- **Scan-in seam.** `UnresolvedFileRelation` (`src/DeltaSharp.Core/Plans/Logical/UnresolvedFileRelation.cs`)
  ...
  (`src/DeltaSharp.Executor/Physical/PhysicalPlan.cs:121`) — the Executor supplies a Storage-backed
...
- `DeltaSharp.Storage`'s `AssemblyName` is registered in **`tools/coverage/coverage-config.json`
  `expectedAssemblies`** and **`tools/security/sca-policy.json` `expectedProjects`** (both **fail-closed**
  gates) the moment the project lands, or the coverage/SCA gates fail closed (per the drift-guard memories
  and the config `expectedAssembliesNotes`/`expectedProjects` comments).
...
- **Decimal:** Parquet `DECIMAL` over `INT32` (≤9 digits), `INT64` (≤18), or `FIXED_LEN_BYTE_ARRAY`/`BYTE_ARRAY` (unscaled two's-complement, **big-endian**). Materialize to DeltaSharp's compact-`long` decimal when precision ≤ 18, else `Int128`, mirroring `ArrowColumnReader.MaterializeDecimal` — including its **fail-closed** guard when an unscaled magnitude exceeds declared precision (`ArrowColumnReader.cs`). Note the endianness difference from Arrow decimal128 (little-endian): the Parquet decoder reads big-endian.
- **Timestamp:** Spark writes `TIMESTAMP` as `INT64` micros with `isAdjustedToUTC=true` (instant). DeltaSharp stores timestamp as epoch-micros `long` (`ColumnVector.GetValues<long>()`), a direct copy. Legacy `INT96` (Julian-day nanos) and `TIMESTAMP_MILLIS` are rebased per the `int96RebaseMode`/`datetimeRebaseMode` reader options already recorded on the plan (`read-door.md:251`).
...
- **`metaData`** — `id`, `name`, `description`, `format{provider, options}`, **`schemaString`** (the Spark schema JSON that `SchemaJson.cs` already round-trips — its own docstring notes this is "the same representation Delta stores in its transaction log"), `partitionColumns[]`, `configuration{}`, `createdTime`. Column-mapping ids live in the schema's per-field metadata (§2.12.3), which `SchemaJson` already carries as string field metadata.
...
- **`remove`** — `path`, `deletionTimestamp`, `dataChange`, `extendedFileMetadata`, `partitionValues`, `size` (checklist bullet 5; preserved for time travel + VACUUM).
...
If the conditional-create loses (versions `(R, M]` were committed since our read snapshot *R*), read the winning commits, classify, and either **rebase** our actions onto `M+1` and retry, or **abort** with a precise conflict. Cell = outcome of *our op* against a *concurrently committed op*; abort reasons use Delta's exception vocabulary.

| Our op ↓ / Concurrent → | Append (`add`, `dataChange=true`) | Overwrite (`remove`+`add`) | Metadata / schema change | Partition overwrite (pred *P*) | Delete (`remove` by pred) | Compaction (OPTIMIZE, `dataChange=false`) | Protocol upgrade |
|---|---|---|---|---|---|---|---|
| **Append** | ✅ rebase | ⚠️ conflict if overwrite scope covers our target | ❌ `MetadataChanged` | ✅ if our files ∉ *P*; else `ConcurrentAppend` | ✅ (our new files independent) | ✅ (`dataChange=false`, no logical overlap) | ❌ `ProtocolChanged` (re-check features) |
| **Overwrite (all)** | ⚠️ `ConcurrentAppend` (their new files not in our replace set) | ❌ conflict | ❌ `MetadataChanged` | ❌ conflict | ⚠️ `ConcurrentDelete` | ⚠️ conflict (compaction rewrote files) | ❌ `ProtocolChanged` |
| **Partition overwrite (*P*)** | ✅ if their `add` ∉ *P*; else `ConcurrentAppend` | ⚠️ if overlap | ❌ `MetadataChanged` | ⚠️ if same partitions | ⚠️ if delete touched *P* | ⚠️ if compaction rewrote files in *P* | ❌ `ProtocolChanged` |
| **Delete (by pred)** | ✅ if appended files don't match pred; else `ConcurrentAppend` (append-into-delete-scope) | ⚠️ conflict | ❌ `MetadataChanged` | ⚠️ if overlap | ⚠️ `ConcurrentDeleteDelete` if same files | ⚠️ `ConcurrentDeleteRead` (compaction removed our targets) | ❌ `ProtocolChanged` |
| **Compaction (OPTIMIZE)** | ✅ if append disjoint from compacted files; else `ConcurrentDelete` | ⚠️ conflict | ❌ `MetadataChanged` | ⚠️ if overlap | ⚠️ `ConcurrentDelete` if delete removed compacted files | ⚠️ conflict (both rewrote) | ❌ `ProtocolChanged` |
| **Schema evolution / metadata** | ❌ conflict | ❌ conflict | ❌ `MetadataChanged` | ❌ conflict | ❌ conflict | ❌ conflict | ❌ `ProtocolChanged` |
| **Protocol upgrade** | ❌ conflict | ❌ conflict | ❌ conflict | ❌ conflict | ❌ conflict | ❌ conflict | ❌ conflict |

✅ = rebase-and-retry with no logical conflict; ⚠️ = conflict iff file/partition/predicate scopes overlap; ❌ = always conflict (metadata/protocol take the table exclusively). This covers concurrent metadata/protocol changes, partition overwrites, deletes, compaction, schema evolution, and read predicates (checklist *ACID* bullet 4). Compaction is deliberately `dataChange=false` so it does not conflict with concurrent appends unless they touch the *same* files (checklist *Maintenance* bullet 1).
...
| `mergeSchema` | Opt-in (writer option / recognized reader option, `read-door.md:251`): union new columns into the table schema, committed as a `metaData` action **in the same transaction** as the data. |
...
- Each `StructField` carries `delta.columnMapping.id` and `delta.columnMapping.physicalName` in its field metadata — which `SchemaJson` already serializes/round-trips as string field metadata (`SchemaJson.cs`, `WriteMetadata`/`ReadMetadata`). Parquet is read/written **by field-id** (id mode; §2.9), so a logical **rename** only rewrites the `metaData` schema, not the files; a **drop** removes the field from the logical schema while the physical column remains unreferenced in existing files.
...
The deterministic simulation (§3.4) records a **history**: an ordered log of operations `{ invoke, ok/fail/info }` tagged with process id, op type (`read@v`, `commit target N`, `retry(txn)`), returned version, observed row multiset, and real-time bounds. A checker validates the history against the invariant catalogue: monotonicity (I1), single-winner (I2), snapshot isolation (I4 — a read that returned pinned@N must reflect no commit whose version `> N`), read-your-writes (I5), idempotent publication (I6), and absence of anomalies (I8: lost update / dirty read / duplicated committed rows). DeltaSharp Delta targets **snapshot isolation** (not full serializability) — the legal-history predicate encodes SI explicitly, and the checker marks *insufficient* rather than *legal* when it cannot classify a history [persona rule 12]. Because histories come from the seeded simulator, each is byte-reproducible (no wall-clock nemesis required). **Discharges:** [21 §Correctness oracles: *"Jepsen-style histories check Delta version monotonicity, snapshot isolation, read-your-writes, idempotent publication, and illegal anomalies"*]; [17 §Optimistic commits]; 05.3.1 AC1.
...
This is the deterministic crux for I2, and it **must** use the house TOCTOU pattern from `SparkSessionConcurrencyTests` / `RuntimeConfig.StopRaceProbe` — not a probabilistic stress loop.
...
- **Oracle.** Exactly one of `{W_A, W_B}` observes success at N; the other observes a precise, retryable conflict (I2). The winner's rows are the committed content of N; the loser either retries onto N+1 with **no** duplication (I6) or fails with the documented conflict class. As in F1/B2, the oracle observes state **strictly inside the in-lock window**, so it never false-positives on a legitimate post-window commit.
- **Two race layers — both proven.** In-process commit coordination is proven by the probe-seam above (a real lock-held publication race, not a simulated one). The **cross-process / backend** single-winner guarantee ultimately rests on the adapter's `If-None-Match`/put-if-absent atomicity, so a companion test races two *independent* adapter instances against the same commit key via Harness A's *conditional-create-race-lost* fault and asserts the backend admits exactly one winner. A backend that cannot provide the primitive is rejected at commit time, not worked around [17 §Optimistic commits: *"rejects adapters that cannot provide it"*].
...
- **Cross-epic handoff (mapped elsewhere, not a gap here).** 05.3.2 AC3's *speculative/repeated task output → one committed output* is proven **deterministically** at the commit-coordinator model here (FI-B/CP1/CP3, OR-d), but live executor-pod double-publish under real scheduling is EPIC-08 distributed-execution + integration chaos; live Kubernetes pod-kill/drain gameday execution hands to `cloud-native-site-reliability-engineer` and EPIC-10. Happy-path Parquet/commit throughput hands to `performance-benchmarking-engineer`. Delta transaction-semantics and Parquet-layout *design* remain owned by `delta-storage-format-engineer`; this section verifies, it does not define, those contracts.
...
- **Bounded row-group buffering.** Buffering is bounded by the reservation ceiling; the reader does not hold the whole file resident. Output batches follow the engine's fixed `OutputBatchRows = 1024` chunking convention (`InterpretedAggregateStream.cs`), so decode scaffolding is `schema-width × fixed-cap`, bounded and known at plan time.
...
- **Decompression-bomb / huge-row-group guard (security).** A malicious or pathological file advertising a tiny compressed page that inflates to gigabytes, or an oversized row group, is contained by the **same memory-reservation ceiling**: the decompressed footprint is reserved first, so an over-budget expansion is refused with a deterministic storage error rather than exhausting the pod. This is both a performance and a **security** control on shared/multi-tenant executors (checklist 05 security; checklist 14 tenant isolation) — critical because storage decodes untrusted tenant bytes.
...
Gated storage metrics, mapped to checklist-22 cadences. All pre-merge/nightly gates are **deterministic and cluster-free**: seeded generators + adapter-latency simulation + in-process spill store, with the §4.2.1 fingerprint recorded so only like-for-like runs are compared. Comparison is benchstat-style (repeated runs, CI/nonparametric, noise floor) with **change-point detection** across commits/runtime upgrades/dataset changes.
...
| Multi-tenant noisy-neighbor (victim p99/p99.9) | shared-executor/PVC/object-store matrix | nightly | victim-tenant tail inflation beyond threshold |
...
- **Tier forcing for reachable parity.** Fast paths are dispatched through the existing tier gates (`KernelTier` in `src/DeltaSharp.Engine/Columnar/KernelTier.cs`; `NullMaskTier` for validity) whose `Auto` value keeps the production `IsHardwareAccelerated` dispatch (and its AOT dead-code elimination of unsupported tiers), while the forcing values (`Scalar` / `Vector128` / `Vector256`) make each SIMD body **deterministically reachable and parity-tested even on hosts where `Auto` constant-folds it away** (e.g. the AVX2 path on an arm64 CI box). Decode kernels reuse this pattern so a fast path is never vacuously green.
...
- **C-DECODE — bounded, validated decode.** Enforce ceilings on decompressed size, decompression ratio,
  ...
  **no partial rows** (checklist 17 "Parquet correctness"; STORY-05.1.1 AC4). The decompression-bomb
  guard reuses the memory-reservation ceiling (§4.3) so a malicious file is refused, not OOM'd — a
  security control on shared/multi-tenant executors (checklists 05, 14).
...
- **C-SCOPE.** Paths are canonicalized and confined to the table root / tenant prefix; credential-bearing
  or path-traversal URIs are rejected (deny-by-default). Listing is authorized and **never** the source of
  active-file truth (the committed log is — checklist 17); no cross-tenant listing. Statistics in the log
  respect the data's classification (§5.1). (STORY-05.1.3 AC3; checklist 14.)
...
| **C-SCOPE** | Path canonicalization + table-root/tenant-prefix confinement; authorized listing; snapshot from committed log, not directory listing | §5.5; 14 "Storage and shuffle isolation"; 17 "Delta log protocol"; STORY-05.1.3 AC3 |
...
| **PDX-D** | Parquet decoder · D | Decompression bomb / huge row-group / absurd page-dict / deep nesting → OOM / CPU exhaustion of pod | C-DECODE ceilings + decompressed-ratio cap + bounded depth + codec allowlist + deterministic fail-closed (17; STORY-05.1.1 AC4) | Ceilings need tuning; a within-limit-but-costly file still costs — bounded by pod limits + per-query memory budget (`operator-spill.md` §7.1) → **operator / query-execution** |
...
| Data freshness / staleness | wall-time between latest committed `table.version` and reader snapshot version (reported as a log/span exemplar, not a labeled metric) | FEAT-05.2 | `<!-- TBD -->` |
...
**New bounded labels this epic mints** (documented in conventions as "not yet minted"; each is added to `DeltaSharpTelemetry` **and** its mutation test in the *same PR* as the instrument that first emits it — the fail-closed registry rule):
...
| --- | --- | --- | --- | --- | --- | --- |
| Commit-conflict storm | Writers livelock; write throughput collapses | `commit.conflicts` rate + `commit.attempts` p95; retry exhaustion (`outcome=failure` after max attempts) | conflict rate ≫ baseline **or** any retry-exhaustion (`<!-- TBD -->`) | **SEV-2** (page) | `delta-storage-format-engineer` | `docs/runbooks/delta-commit-conflict-storm.md` `<!-- TBD -->` |
...
| `sca` (`security.yml`) | **Register `DeltaSharp.Storage` (+`DeltaSharp.Storage.Tests`) in `tools/security/sca-policy.json` `expectedProjects`** — fail-closed provenance allowlist; a missing project name fails CI. `failOnSeverity: high`. |
...
| `aot.yml` (NativeAOT publish) | **Keep green if Storage becomes reachable from `DeltaSharp.Executor`** (it will — the executor reads/writes tables). Storage must be trim/AOT-annotation-clean with **no dynamic code** (Parquet decode must not require `Reflection.Emit`/`Expression.Compile`; those stay banned per ADR-0001/`BannedSymbols.txt`), so `dotnet publish -p:PublishAot=true -warnaserror` stays clean (ADR-0014; checklist 10). |
...
- **D-5 · Settle the Engine lock-file drift (#468), register the new assembly in both fail-closed gates,
  ...
  (`persona:delta-storage-format-engineer`) when the assembly lands.
