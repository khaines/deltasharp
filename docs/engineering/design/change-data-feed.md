...
**Overwrite (`DeltaWriteTarget.OverwriteAsync`).** No change files. Static/dynamic overwrite already commits `remove(old, dataChange=true) + add(new, dataChange=true)` atomically; a reader derives `delete` (old rows) + `insert` (new rows). *Caveat (Spark parity nuance):* Spark can emit `cdc` for `replaceWhere`-style predicated overwrites to avoid surfacing unchanged rows; DeltaSharp's overwrite is whole-table/whole-partition, so the derived representation is faithful. Recorded as an open question (§9 Q3) should predicated overwrite land. **Durability caveat (retention).** The `delete` side is *derived* by reading the **removed** data files at CDF-read time. Those files are ordinary `remove.path` **tombstones**, which `OrphanCleanup` **already** retention-protects while removed at/after the VACUUM cutoff (`deletedFileRetentionDuration`); since the CDF-readable window is bounded by that same retention (§2.6), an overwrite's removed files are **already protected whenever the range is CDF-readable** — no new work. The only exposure is a **sub-retention VACUUM override**, which is the general aggressive-VACUUM caveat that already breaks time-travel (not CDF-specific). INV C7 (§3.3) pins that this protection holds across the CDF window. Append is unaffected (its `add` files stay active); DELETE is unaffected (it materializes `cdc`, and precedence suppresses derivation).
...
| **#489** VACUUM retention protection | `_change_data/` `cdc` files must be VACUUM-protected (removed data files are already `remove.path`-tombstone-protected; §2.12); blocks generation GA (§9 Q5). |
...
- **VACUUM / retention:** the one genuinely-new protection CDF needs is **`_change_data/` `cdc` files** referenced by an in-window `cdc` action — they are referenced by a **non-`add.path`** field, so `OrphanCleanup` does not protect them yet (exactly what **#489** now tracks; the DV `.bin` half **already landed** — `OrphanCleanup` protects active + tombstone DV sidecars today). The overwrite `delete` side's **removed data files** need **no** new work: they are ordinary `remove.path` **tombstones** already retention-protected by `OrphanCleanup` within `deletedFileRetentionDuration`, and the CDF window ≤ that (INV C7, §2.5). CDF generation **depends on** #489's `_change_data/` protection landing with (or before) it; flagged §9 Q5.
...
| CDF-HP-05 | `_commit_timestamp` stamping | Each change row's `_commit_timestamp` == the version's resolved **commit-file (`<N>.json`) mtime** — the same value time-travel-by-timestamp resolves (§2.8), set deterministically in the fixture; **not** `commitInfo.timestamp`/`TimeProvider` | AC1 |
...
| CDF-HP-10 | **Mixed-endpoint** range: `startingVersion` + `endingTimestamp` (Spark parity) | The two endpoints resolve **independently** to the expected `[startVersion, endVersion]` (§2.9) and are **accepted**, not rejected | AC2 |
...
| CDF-EE-04 | A **single endpoint** carries both a version and a timestamp (e.g. `startingVersion` AND `startingTimestamp`) | `ArgumentException` (per-endpoint version-xor-timestamp, §2.9; mixing *across* endpoints is legal — CDF-HP-10) | AC2 |
| CDF-EE-05 | **Non-read-compatible** schema change within the range — an **absent required (non-nullable)** output column (cannot null-fill) or an **incompatible retype** no widening rule covers (a column *dropped* before `endVersion` is **not** a failure — it is simply not projected, §2.8) | `DeltaReadSchemaEvolutionException`; never fabricates values | AC4 |
...
| CDF-EE-08 | A root-confined `cdc` file whose **embedded schema mismatches that version's write schema** (hostile/inconsistent log) — distinct from legitimate cross-version evolution (CDF-EE-05) | The decoded `cdc` leaf schema is validated against **that version's log-resident `MetadataAction` schema** (the trusted authority) **before** any row is yielded or reconciled; a mismatch fails closed (classified CDF/tampering error) — never projects attacker-chosen columns. A valid file then goes through normal per-version→output reconciliation (§5.2, CDF-EE-05) | AC1 |
...
| C7 | Derivation-file retention | Every file an **in-window implicit derivation** must read (a removed data file behind an overwrite `delete`) is retention-protected for the whole CDF-readable window — `OrphanCleanup` protects the `remove.path` tombstone within `deletedFileRetention` and the CDF window ≤ that, so VACUUM never reclaims it while a range referencing it is readable (§2.5). |
...
| AC2 | CDF read over a version range returns only in-range changes in commit order | CDF-HP-04/06/10; CDF-EE-03/04; INV C5 |
| AC3 | CDF disabled / historical range → clear unsupported-range error | CDF-EE-01/02/06/09; §2.7 |
| AC4 | Schema evolution during a CDF range follows documented compatibility rules | CDF-HP-07; CDF-EE-05; §2.8 |
...
**Gates (checklist 22; parent §4.6):** a **structural** gate — *files opened ⊆ in-range changed files, zero unchanged/snapshot files* (§4.2), adapter-counted like the parent's COMMIT-1/MAINT-1 gates; and an **allocation** gate on stamping (asserts the metadata-column materialization is `O(1)` allocation **count** per batch on the warmed reuse path — *not* a per-row bytes-allocated gate — so an allocation-count regression fails even if wall-clock is flat). Comparison is benchstat-style with change-point/drift detection; each run records the pinned-environment fingerprint (parent §4.2.1: SDK/runtime, GC mode, commit SHA). No absolute numbers are committed until calibrated on the reference environment.
...
**Privacy note (GDPR erasure) — the standout CDF concern.** CDF *retains a copy of deleted rows* in `_change_data/` (and their identifying `cdc.partitionValues` in the commit log) until they age out and are VACUUMed. So a "right to be forgotten" delete is **not physically erased until the change files are vacuumed**: the **effective erasure floor** for a deleted row's body is `deletedFileRetentionDuration`, and for a subject whose PII sits in a **partition column** it is the **longer** `max(deletedFileRetentionDuration, logRetentionDuration)` — because `cdc.partitionValues` persist in the append-only commit **log** under *log* retention, not the data-VACUUM window (§5.1 table). (This erasure *floor* — how long PII persists — is the **`max`** of the windows; it is distinct from the CDF-*readable* window of §2.6, which is the **`min`**, since a feed is readable only while **both** the log and the data survive.) Both are operator-configurable and, if retention is long, potentially weeks/months. Two consequences: (1) an operator with an erasure SLA must **bound both retentions** accordingly (CDF *raises the effective erasure floor*); (2) time-based VACUUM offers **no subject-targeted purge** — a specific subject's rows cannot be removed ahead of the retention interval, and even a targeted `_change_data/` purge cannot erase already-committed `cdc.partitionValues` without log rewrite/checkpoint cleanup (append-only log) — so a **targeted CDF purge** affordance is an open question (§9 Q7). Handoff to `privacy-compliance-grc-lead`; risk in §8.3.
...
- **`cdc` content/schema validation.** Path confinement bounds only *where* a `cdc` file lives, not *what it declares*, so content is validated separately: a `cdc` file's decoded (physical/column-mapped) leaf schema is checked against **that version's log-resident `MetadataAction` schema** (the trusted authority — never a file-derived schema) **before** any row is yielded or reconciled; a mismatch fails closed (§3.2 CDF-EE-08). Only then does a valid file pass through the normal per-version→output reconciliation (which independently fails closed on a genuine incompatibility, CDF-EE-05). The synthesized **`_change_type`** token is also validated against the allowed Delta domain (`insert`/`delete`/`update_preimage`/`update_postimage`), so a schema-valid hostile file cannot surface deletes as inserts.
...
| **Tampering** (content) | Root-confined `cdc` file whose **embedded schema mismatches** the version — surfaces fabricated columns as authentic change data | Validate each `cdc` file's decoded leaf schema against **that version's log-resident `MetadataAction` schema** (trusted authority, not file-derived) **before** reconciliation/yielding, plus the `_change_type` domain; fail closed (§5.2, §3.2 CDF-EE-08) | Low |
| **Information disclosure** | Change files leak deleted-row PII beyond erasure SLA | Same-as-data classification; VACUUM protection + **bounded** erasure floor (`max(deletedFileRetentionDuration, logRetentionDuration)` for partition-column PII, §5.1) + targeted-purge open question (§5.1, §9 Q7, #489) | Medium → privacy handoff |
...
| **Repudiation** | Which version produced a change | `_commit_version` is **authoritative** (log ordering); `_commit_timestamp` is **best-effort** and attacker-influenceable on a hostile log (derived from the `<N>.json` commit-file mtime, storage-stamped, not crypto-bound; §2.8) — not for security/audit ordering | Low (version) / accepted (timestamp) |
...
| Overwrite `delete` side unreadable after an **aggressive sub-retention VACUUM** override | Low (general aggressive-VACUUM caveat — already breaks time-travel; removed files are `remove.path`-tombstone-protected within `deletedFileRetention`, INV C7) | Not CDF-specific; documented (§2.5, §2.12) |
...
| Deleted-row + partition-value retention vs. GDPR erasure | Medium | Erasure floor == `max(deletedFileRetentionDuration, logRetentionDuration)` (partition-column PII, §5.1); targeted-purge open (§9 Q7); privacy handoff (§5.1) |
...
Builds/tests/`dotnet format` pass; checklists **17** (Delta storage format), **19** (data-source connectors), **03a** (.NET standards), **04b** (integration) satisfied (story DoD); INV **C1–C9** green; #489 VACUUM protection (`_change_data/` `cdc` files) landed or explicitly gated; `PublicAPI.Unshipped.txt` updated; follow-ups **#637**/**#638** filed; storage-delta-architecture.md §2.14 cross-reference kept consistent.
...
- **Q5 — VACUUM protection sequencing (#489).** CDF generation must not GA before **`_change_data/` `cdc` files** are VACUUM-protected (#489's remaining scope; DV `.bin` already landed). Overwrite removed data files need **no** new work — they are `remove.path` tombstones already protected within `deletedFileRetention` (INV C7, §2.5). Confirm #489 lands with or before increment (2).
