...
| Type | Kind | Role |
| --- | --- | --- |
| `SparkSession` | `sealed class : IDisposable` | The session: config access, doors, lifecycle, static active/default tracking. |
...
| `Set(string key, string value)` (+ `bool`/`long` overloads) | `set(...)` | Mutates the live runtime conf (Spark semantics). |
...
**Synchronization.** `GetOrCreate`, and the mutation of the default session inside `Stop()`, run
inside a single private static **monitor lock** (`_globalLock`) so concurrent `GetOrCreate` calls on
different threads cannot create two competing default sessions — the first wins, the rest reuse it.
Per-session lifecycle state is an `int` flag mutated with `Interlocked` so `Stop()`/`Dispose()` are
race-free and idempotent. `RuntimeConfig` is backed by a thread-safe map so concurrent `Conf` reads
are safe.
...
4. **Create.** Else construct a new `SparkSession` from the builder's config, parse and record the
   [execution backend](#backend-selection-recording), publish it as both the default and this
   thread's active session, and return it.
...
every door — `Read`, `Sql`, and `Conf.Set` — and will back the future `CreateDataFrame` (#158). On a
stopped/disposed session it throws `SessionStoppedException`. `Stop()`/`Dispose()` never throw
(idempotent). Conf **reads** never throw.

**`Read`/`Sql` on an *active* session (M1).** The doors exist but their query behavior is delivered
by #158/#159. On an **active** session they throw a documented
...
> **Scope note — `CreateDataFrame`.** AC3 also names "creating a DataFrame". A faithful
> `CreateDataFrame` overload requires public schema/row types (`StructType`, `Row`) that do not yet
> exist in `DeltaSharp.Core` (they arrive with the reader/encoder work). #157 therefore wires the
> shared lifecycle guard into the two DataFrame-producing doors that *can* be expressed now — `Read`
> and `Sql` — and AC3 is verified through them. `CreateDataFrame` (#158) will reuse the identical
> `EnsureNotStopped` guard, so the error model is established here and inherited unchanged.
...
(`Config("spark.deltasharp.execution.backend", "interpreted")`); `GetOrCreate` parses it **once** at
creation into the `ExecutionBackend` enum exposed by `SparkSession.ExecutionBackend`. An unrecognized
value throws a deterministic `ArgumentException` from `GetOrCreate` naming the offending value and the
allowed set. Absent the key, the default is `ExecutionBackend.Auto`.

**Zero work at record time.** Parsing the string into the enum and storing it touches **no** engine
type and performs **no** backend construction, kernel warm-up, or capability probe. The recorded enum
is inert until an action consumes it (`SparkSession.ExecutionBackend` is a documented DeltaSharp
...
| --- | --- | --- |
| **AC1** | Builder app name + key/value config; `GetOrCreate` returns a usable session exposing the values **without executing**. | `Builder_AppNameAndConfig_AreExposedThroughConf`; `GetOrCreate_ReturnsUsableActiveSession_WithoutExecuting`; `Builder_ConfigOverloads_StoreInvariantStrings` |
| **AC2** | Existing active session + equivalent config → **same** active session; config applied to its runtime conf. | `GetOrCreate_WithActiveSession_ReturnsSameInstance`; `GetOrCreate_AppliesNewConfig_ToExistingSession`; `GetActiveSession_AfterGetOrCreate_ReturnsCreatedSession` |
| **AC3** | Stopped/disposed session: `Read`/`Sql` (and DataFrame creation) → deterministic lifecycle error. | `Read_OnStoppedSession_ThrowsSessionStopped`; `Sql_OnStoppedSession_ThrowsSessionStopped`; `Read_OnDisposedSession_ThrowsSessionStopped`; `Sql_OnDisposedSession_ThrowsSessionStopped`; `SessionStopped_Message_IsDeterministicAndNamesApp`; `Read_OnActiveSession_ThrowsNotSupported_NotLifecycle` |
| **AC4** | Backend config recorded at creation for later execution **without initializing work**. | `ExecutionBackend_DefaultsToAuto`; `Config_ExecutionBackend_Interpreted_IsRecorded`; `Config_ExecutionBackend_IsCaseInsensitive`; `Config_ExecutionBackend_InvalidValue_ThrowsAtGetOrCreate`; `GetOrCreate_RecordsBackend_WithoutTouchingEngine` |
| Lifecycle | Stop/Dispose idempotent; clears active/default; Conf read-after-stop; Conf.Set-after-stop throws. | `Stop_IsIdempotent`; `Dispose_StopsSession`; `Stop_ClearsActiveAndDefault`; `Conf_ReadsRemainValid_AfterStop`; `Conf_Set_AfterStop_ThrowsSessionStopped`; `ActiveSession_SetClear_RoundTrips` |
