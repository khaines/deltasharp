...
| `RelationalGroupedDataset.Count()` | `RelationalGroupedDataset.count()` | `DataFrame` — `Agg(Count(Lit(1)).As("count"))` |
...
`Expression`, `LogicalPlan`, and `DeltaSharp.Plans.*` never appear on the public surface. The five
members above are the only additions to `PublicAPI.Unshipped.txt`.
...
- **Bare aggregate → analyzer names it.** A bare aggregate such as `Functions.Sum(Functions.Col("v"))`
  ...
  DeltaSharp defers that to STORY-04.5.2 (#171), exactly as Spark defers it to `ResolveAliases` over
  `UnresolvedAlias`. The API never computes the name eagerly (it would have to inspect/execute the
  expression). Verified by `Agg_BareAggregate_IsLeftUnaliased_ForAnalyzerNaming`.
- **`Count()`** is the one door that fixes a name: it is `Agg(Count(Lit(1)).As("count"))`, matching
...
over a registered catalog: `df.GroupBy("dept").Agg(Lit(1).As("marker"))` resolves to an `Aggregate`
...
aggregate position because aggregate-**function** resolution is out of scope (see §6).
...
- leaves aggregate **function resolution** and **pretty-naming** to the analyzer. Today an
  `UnresolvedFunction` (`sum`/`count`/…) is *not* resolved: `Analyzer.CheckAnalysis` throws a
  deterministic `AnalysisException` when one survives. `Analyzer_RealAggregateFunction_ReportsDeferredResolution_NotApiExecution`
  pins that the **analyzer** is the gate (a real `Sum(...)` aggregate throws `AnalysisException` on
  `Resolve`, and nothing is read/executed).
...
complete and lazy, and the analyzer boundary is explicit.
...
| **AC2** | `Agg` (grouped) builds `Aggregate` with grouping + retained keys ⧺ aggregate exprs and Spark aliases; global `df.Agg` builds `Aggregate` with empty grouping. | `Agg_BuildsAggregateWithGroupingAndRetainedKeysThenAggregate`, `Agg_HonorsUserAliasOnAggregateOutput`, `Agg_BareAggregate_IsLeftUnaliased_ForAnalyzerNaming`, `Agg_MultipleAggregates_AppendInOrderAfterRetainedKeys`, `Count_BuildsAggregateWithNamedCountAfterRetainedKeys`, `GlobalAgg_BuildsAggregateWithEmptyGrouping`, `GlobalAgg_IsEquivalentToGroupByNoKeysThenAgg` |
| **AC3** | Invalid aggregate input is reported by the **analyzer**, not the API; the API builds a well-formed unresolved plan (deep validation deferred to #171). | `Analyzer_RealAggregateFunction_ReportsDeferredResolution_NotApiExecution`; structural derivation via `Analyzer_DerivesAggregateOutput_AsGroupingAttributesThenAggregateAliases` |
...
| **Guards** | Null/empty argument rejection. | `GroupBy_Null*`/`GroupBy_Empty*`, `Agg_Null*`, `GlobalAgg_NullFirstExpr_Throws` |
