...
| Storage telemetry | `DeltaStorageTelemetry` ships the `DeltaSharp.Delta` commit/VACUUM/OPTIMIZE/DELETE instruments and spans, the snapshot-reconstruction `deltasharp.delta.checkpoint.fallbacks` counter (#772), plus the `LoggerMessage` log sites in `src/DeltaSharp.Storage/Diagnostics/` | Adapter and Parquet I/O instruments on the `DeltaSharp.Storage` meter/source, which are created (to fix the naming) but carry no instruments yet |
