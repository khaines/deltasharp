...
| `ColumnMapping.ValidateColumnMappingSchema` foreign-`nested.ids` **reject** | `ColumnMapping.cs:502-513` | **Mode-gated parse + validate.** The parse+validate path runs **only when `mode == Id && field is array/map`**. Under **name mode** on a field of **ANY** shape (scalar, struct, **or array/map**), **or** id mode on a non-array/map field, the presence of `nested.ids` is the **unconditional fail-closed reject**, exactly as #676's `ColumnMapping.cs:502-513` did (nested.ids is meaningful ONLY under id mode on an in-scope array/map; a name-mode array/map has no field_ids to bind, so a stray/foreign `nested.ids` is rejected, **never** accepted-and-ignored). Under **none mode** the schema validator short-circuits at method entry (none mode carries/consults no column-mapping metadata), so a stray `nested.ids` is an **inert no-op** — Delta-faithful (`getColumnMappingMetadata` strips it under `NoMapping`) and structurally safe (none-mode read binds by logical name, never by `field_id`). When the id-mode array/map gate applies, parse+validate (§2.3): parse the JSON object; **require every value to be `MetadataValueKind.Long`** (a non-Long value — Double/String/Bool/Null/nested-object — fails closed as a **typed** `DeltaProtocolException` **before** the range check — Finding 3); require keys to be exactly `<physicalName>.element` (array) or `<physicalName>.key`+`<physicalName>.value` (map) matching the field's actual shape; require values in `[1, maxColumnId]` (and the `[1, int.MaxValue]` footer bound); require no duplicate id (global uniqueness extended over interior ids). A malformed/foreign-shape/wrong-mode `nested.ids` fails closed (never silently ignored). |
...
16b. `NameMode_ArrayMap_WithForeignNestedIds_FailsClosed` and `NoneMode_ArrayMap_WithForeignNestedIds_FailsClosed`
    (Finding 2) — a **name-mode** `array<scalar>`/`map<scalar,scalar>` carrying a stray/foreign `nested.ids`
    **fails closed** exactly as #676's unconditional reject (`ColumnMapping.cs:502-513`): it **is** an
    array/map, so the §3.16 non-array/map check misses it, and name mode has **no** field_ids to bind, so
    accepting-and-ignoring would **regress #676's fail-closed guarantee**. **None mode is DIFFERENT (impl
    errata, council-endorsed at #854 R2):** `ValidateColumnMappingSchema` short-circuits `mode == None` at
    method entry — none mode carries no column-mapping metadata and never consults `field_id`/`nested.ids`
    on read — so a stray/foreign `nested.ids` on a none-mode field is an **inert no-op**, matching Delta's
    own behavior (`getColumnMappingMetadata` **strips** `nested.ids` under `NoMapping`, it does not reject).
    This is structurally safe (none-mode read binds purely by logical name; a stray id can never
    mis-attribute an interior leaf) and is verified by the test asserting the none-mode no-op and contrasting
    it with the name-mode reject. Precise gate (§2.4): parse+validate runs **only** when
    `mode == Id && field is array/map`; a `nested.ids` under **name mode** (any shape) or under **id mode on
    a non-array/map** field is the **unconditional fail-closed reject**; under **none mode** it is inert
    (stripped/ignored, Delta-faithful).
