...
- [ ] The reconcile gate's `settings-permissions` check passes: destructive-spelling deny entries present; no enumerated mutating prefix, wildcard, or tool-wide `Bash` grant in `allow`; no `defaultMode` bypass, project `hooks`, or `additionalDirectories`. Entries outside that enumeration are reviewed by hand.
