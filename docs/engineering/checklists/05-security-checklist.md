...
- [ ] The gate's `tracked-startup-config` check passes: no tracked root `.mcp.json` declaring MCP servers (their `command` is spawned at CLI launch, unprompted), no tracked `.claude/settings.local.json` (it is honoured like `settings.json` and must stay gitignored per-machine state), and no symlinked directory or file under `.claude/agents`, `.claude/commands`, or `.claude/skills` (Claude Code follows links the gate's walks do not, so the target would ship unreviewed).
