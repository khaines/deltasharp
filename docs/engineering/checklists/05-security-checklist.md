...
- [ ] The gate's `tracked-startup-config` check passes: no tracked root `.mcp.json` declaring MCP servers (their `command` is spawned at CLI launch, unprompted), no tracked `.claude/settings.local.json` (it is honoured like `settings.json` and must stay gitignored per-machine state), and no tracked symlink under `.claude/` or at `.mcp.json`/`.claude/settings.local.json`/`.claude/settings.json` — the rule is decided by the git mode (`120000`), not by what the link resolves to in this checkout, so a **dangling** link (one pointing at `bin/`, `obj/` or `artifacts/`: absent in CI, present after any local build) cannot hide, and Claude Code follows links the gate's walks deliberately do not.
