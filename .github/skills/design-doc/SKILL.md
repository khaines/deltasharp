---
name: design-doc
description: Generates comprehensive component design documents from GitHub issues. Orchestrates specialist agent personas — Architect for structure and logic, Security SME for threat modelling, SRE for observability and rollout — to produce a complete design document following the standard template. Creates a worktree branch, commits the design doc, opens a PR, and invokes the review-fix-loop skill for automated quality improvement.
---
<!-- GENERATED from .claude/skills/design-doc/SKILL.md by tools/aiconfig/generate-copilot.py — do not edit; edit the source and re-run with --write. -->
