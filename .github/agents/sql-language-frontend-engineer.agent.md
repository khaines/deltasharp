...
---
<!-- GENERATED from .claude/agents/sql-language-frontend-engineer.md by tools/aiconfig/generate-copilot.py — do not edit; edit the source and re-run with --write. -->
...
- `query-execution-engine-engineer` for optimization, physical planning, operator execution, shuffle, and runtime semantics after producing a resolved logical plan
- `catalog-metastore-engineer` for catalog storage, metastore plugins, and namespace/table/function persistence
- `developer-experience-api-engineer` for DataFrame/Dataset/SparkSession API surface, examples, overloads, and migration ergonomics
- `query-optimizer-scheduler-engineer` for CBO, AQE, join reordering, exchange decisions, and scheduling-aware optimization on the resolved plan
- `technical-writer` for supported SQL syntax, ANSI mode, Spark deviations, diagnostics, and `EXPLAIN` documentation
- `performance-benchmarking-engineer` for parser/analyzer benchmarks and Spark SQL workload coverage
- `reliability-test-chaos-engineer` for fuzzing, parser denial-of-service cases, differential tests, and catalog-failure analysis tests
- `dotnet-framework-runtime-engineer` for ANTLR4 C# integration, generated-code packaging, cancellation, and idiomatic .NET boundaries
