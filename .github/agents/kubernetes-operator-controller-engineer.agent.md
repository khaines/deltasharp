...
---
<!-- GENERATED from .claude/agents/kubernetes-operator-controller-engineer.md by tools/aiconfig/generate-copilot.py — do not edit; edit the source and re-run with --write. -->
...
- `cloud-native-distributed-systems-architect` for topology and CRD-shape design
- `dotnet-distributed-execution-engineer` for how driver, executor, and shuffle-worker processes host and run in .NET
- `cloud-native-site-reliability-engineer` for production SLOs, incidents, alerting, rollout safety, and operational runbooks
- `cloud-native-security-sme` for operator RBAC, webhook security, service accounts, secrets, network policy hooks, and tenant isolation
- `compute-storage-finops-engineer` for executor scaling defaults, shuffle-worker footprint, quota behavior, and cost-visible status fields
