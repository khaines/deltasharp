...
internal static class DatasetSchema
{
    ...
    /// ordered <see cref="StructType"/>. Properties are ordered by
    /// <see cref="MemberInfo.MetadataToken"/> (declaration order within the type), so the derived
    /// schema is deterministic across runs rather than depending on the unspecified order of
    ...
    /// properties).</returns>
    /// <exception cref="UnsupportedTypedExpressionException">A property's CLR type has no ADR-0008
    ...
    private static DataType? MapClrType(Type type)
    {
        if (type == typeof(bool))
        {
